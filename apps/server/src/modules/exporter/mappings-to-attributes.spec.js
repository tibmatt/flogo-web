...
describe('exporter.mappings-to-attributes', () => {
  ...
  test(
    'Should prepare the empty attributes when there is no literal assignment mappings',
    () => {
      expect(mappingsToAttributes({
        id: 'task1',
      ...
      }, testSchemas['activityschema1'])).toEqual({
        id: 'task1',
        activityRef: 'activityschema1',
        attributes: [],
        inputMappings: [{
  ...
  test(
    'Should prepare the attributes when there are literal assignments in mappings',
    () => {
      expect(mappingsToAttributes({
        id: 'task1',
        activityRef: 'activityschema1',
  ...
  test('Should not process anything for a subflow type tasks', () => {
    ...
    }, testSchemas[REF_SUBFLOW])).toEqual({
      id: 'task3',
      activityRef: REF_SUBFLOW,
      attributes: [{name: 'attr1', type: 'number'}],
      inputMappings: [{
  ...
  test('Should not process anything for a mapper type tasks', () => {
    ...
    }, testSchemas['activityschema3'])).toEqual({
      id: 'task4',
      activityRef: 'activityschema3',
      attributes: [{name: 'attr1', type: 'number'}],
      inputMappings: [{
