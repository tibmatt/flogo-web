...
describe("Service: ImportErrorFormatterService", function(this: {
  ...
  it("Should return the proper validation error label", () => {
    var spyTranslateService = <Spy>this.mockTranslateService.instant;
    spyTranslateService.and.callFake(function(key){
      return key;
    });
    let errorLabel = this.service.formatMessageHeader(mockImportErrorResponse[0].meta.details[0].keyword);
  ...
  it("Should return the proper validation error message", () => {
    ...
    let errorLabel = this.service.formatErrorMessage(mockImportErrorResponse[0].meta.details[2]);
