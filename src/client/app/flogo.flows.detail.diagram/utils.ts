...
export function genBranchLine( opts? : any ) : any {
  let spaceBetweenBranches = 73; //fill the space between two branches
  let defaultSize = {
    svgWidth: 92,
  ...
  let defaultPath = genBranchArrow(
    _.assign( {}, opts, {
  ...
  let svgWrapperDefault = `
  <svg width="${defaultSize.svgWidth}px" height="${defaultSize.svgHeight}px" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
    <defs>
        ___FILTER___
        ___FILL___
    </defs>
  ...
  _.each(SUPPORTED_STATES, (state: string) => {
    let wrapper = (state == 'default') ? svgWrapperDefault : svgWrapper;
