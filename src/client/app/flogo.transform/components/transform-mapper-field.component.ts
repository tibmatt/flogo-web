import { Component, Input, Output, ElementRef, EventEmitter, HostListener, OnChanges, OnInit } from '@angular/core';
...
export class TransformMapperFieldComponent implements OnChanges, OnInit {
  @Input() tile: any;
  @Input() mappings: any;
  ...
  onClick(event) {
    const nativeElement = this._eref.nativeElement;
  ...
  precedingTilesToArray(precedingTiles: any) {
    return Object.keys((precedingTiles || {})).map(tileName => ({
      name: tileName,
      fields: precedingTiles[tileName]
    }));
  }
