...
function flogoFlowToJSON(inFlow) {
    ...
    flowJSON.id = utils_1.flogoIDEncode(flowID);
    ...
    flowJSON.flow = (function _parseFlowInfo() {
        ...
        flow.type = _.get(inFlow, 'type', constants_1.FLOGO_PROCESS_TYPE.DEFAULT);
    ...
    function _traversalDiagramChildren(node, visitedNodes, nodes, tasks, tasksDest, linksDest) {
        if (!_.includes(visitedNodes, node.id)) {
            ...
            _.each(nodesToGo, function (nid) {
                ...
                if (childNode.type
                    === constants_2.FLOGO_FLOW_DIAGRAM_NODE_TYPE.NODE_ADD
                    || childNode.type
                        === constants_2.FLOGO_FLOW_DIAGRAM_NODE_TYPE.NODE_ROOT_NEW) {
                    return;
                }
                if (childNode.type === constants_2.FLOGO_FLOW_DIAGRAM_NODE_TYPE.NODE_BRANCH) {
                ...
                if (_isValidInternalTaskInfo(task)) {
                    var taskInfo = {};
                    taskInfo.id = utils_1.convertTaskID(task.id);
                    ...
                    taskInfo.attributes = _.filter(taskInfo.attributes, function (attr) {
                ...
                if (node.type === constants_2.FLOGO_FLOW_DIAGRAM_NODE_TYPE.NODE) {
                    linksDest.push({
                        id: _genLinkID(),
                        from: utils_1.convertTaskID(node.taskID),
                        to: utils_1.convertTaskID(childNode.taskID),
                        type: constants_2.FLOGO_FLOW_DIAGRAM_FLOW_LINK_TYPE.DEFAULT
                    });
                }
                else if (node.type === constants_2.FLOGO_FLOW_DIAGRAM_NODE_TYPE.NODE_BRANCH && node.parents.length === 1) {
                    var parentNode = nodes[node.parents[0]];
                    var branch = tasks[node.taskID];
                    if (parentNode.type !== constants_2.FLOGO_FLOW_DIAGRAM_NODE_TYPE.NODE_ROOT) {
                        linksDest.push({
                            id: _genLinkID(),
                            from: utils_1.convertTaskID(parentNode.taskID),
                            to: utils_1.convertTaskID(childNode.taskID),
                            type: constants_2.FLOGO_FLOW_DIAGRAM_FLOW_LINK_TYPE.BRANCH,
    ...
    function _parseFlowAttributes(inAttrs) {
        ...
        _.each(inAttrs, function (inAttr) {
            ...
            attr.value = _.get(inAttr, 'value', utils_1.getDefaultValue(inAttr.type));
            if (_.isEmpty(attr.name)) {
                DEBUG && console.warn('Empty attribute name found');
                DEBUG && console.log(inAttr);
                return;
            }
            attr.type = _.get(constants_1.FLOGO_TASK_ATTRIBUTE_TYPE, _.get(inAttr, 'type'), 'string').toLowerCase();
